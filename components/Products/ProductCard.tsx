import { CartIcon, Compare24Icon, CompareIcon, HeartOutline24Icon } from '@/assets/icons'
    ...
    return (
        <div className='w-[270px] mx-[15px] group transition-all !duration-500'>
            <div className={`bg-[#EBEFF3]  w-full flex justify-center items-center h-[270px] rounded-[6px] overflow-hidden relative `}>
                <Image src={`${API}/uploads/${product.image}`} alt={product.name} width={202} height={202} className='w-[80%] h-auto group-hover:scale-[1.1] duration-300 cursor-pointer' ></Image>
                <button className='absolute top-5 right-5 text-gray-500 text-[25px] cursor-pointer'><HeartOutline24Icon /></button>
            </div>
            <p className=' line-clamp-2 text-[14px] text-[#545D6A] mt-[16px] mb-[28px]'>{product.description}</p>
