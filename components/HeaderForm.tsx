                ...
                <div className={`w-full absolute rounded-b-[6px] bg-[#EBEFF3] z-[11] left-0 ${isOpenSearchModal && isLoading || searchData?.length ? 'max-h-[300px] pt-[10px] overflow-y-scroll shadow-lg top-[90%]' : 'h-[0px] pt-[0px] overflow-hidden top-[50%]'} duration-500 bg-[#EBEFF3]`}>
                    {isLoading ? (
                        ...
                        searchData?.map((item: any) => (
                            <div key={item.id} onClick={() => { setIsOpenSearchModal(false); setSearchData([]); setSearchValue(item.name); setCurrentValue(item.name) }} className="w-full cursor-pointer text-left border-t-[1px] border-t-[#B6BABF4D]/60 pl-[40px] py-[14px] hover:bg-[#B6BABF4D]/90 duration-300" >
                                {item.name}
                            </div>
                        ))
                    )}
